...
# (unused now) Old flat mapping kept for reference; safe to delete later.
EXEC_LABELS = {
    "kitchen": "GO_KITCHEN",
    "living_room": "GO_LIVING_ROOM",
    "table": "GO_TABLE",
...
def predict_command(text, tokenizer, model, device, C):
    """Return RAW SLOTS dict for the executor."""
    # Encode EXACTLY like training
    enc = tokenizer(
        text,
        return_tensors="pt",
    ...
    # Return RAW SLOTS for executor to interpret
    slots = {
        "intent": intent,
        "region": region,
        "speed": speed,
...
def main():
    ...
    while not rospy.is_shutdown():
        try:
            ...
            if not cmd:
                continue
            slots = predict_command(cmd, tokenizer, model, device, C)
            rospy.loginfo(f"Predicted Slots: {slots}")
            execute_symbolic(slots)   # executor now receives dict of slots
        except (KeyboardInterrupt, EOFError):
            print("\nExiting...")
            break
        except Exception as e:
